...
private:
    ...
    static void vcan_input(ifnet_t interface, mbuf_t packet);
    static errno_t vcan_ioctl(ifnet_t interface, u_long command, void *data);
    static errno_t vcan_set_bpf_tap(ifnet_t interface, bpf_tap_mode mode, bpf_packet_func callback);
...
bool VirtualCANInterface::create_interface()
{
    ...
    // Use the simpler ifnet_allocate API
    result = ifnet_allocate(NULL, &interface);
    if (result != 0) {
    ...
    ifnet_set_name(interface, "vcan", index);
    ifnet_set_family(interface, IFNET_FAMILY_ETHERNET);
    ifnet_set_type(interface, IFT_OTHER);
    ...
    ifnet_set_softc(interface, this);
    
    // Set function pointers
    ifnet_set_output(interface, vcan_output);
    ifnet_set_ioctl(interface, vcan_ioctl);
...
void VirtualCANInterface::vcan_input(ifnet_t interface, mbuf_t packet)
{
    VirtualCANInterface *self = (VirtualCANInterface *)ifnet_softc(interface);
    self->input_handler(packet);
}
    ...
    switch (command) {
        ...
        case SIOCSIFMTU: {
            ...
            if (command == SIOCGIFMTU) {
                ifr->ifr_mtu = ifnet_mtu(interface);
            } else {
...
errno_t VirtualCANInterface::vcan_set_bpf_tap(ifnet_t interface, bpf_tap_mode mode, bpf_packet_func callback)
{
    // Allow BPF tapping for packet capture
    return 0;
}
